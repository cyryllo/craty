<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Sale listing') }} — {{ $item->name }}</h2>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('items.sale-listing.store', $item) }}" class="bg-white rounded-lg shadow p-6 space-y-4">
            ...
            <div class="flex items-center justify-end gap-3 pt-2 border-t border-gray-100">
                <a href="{{ route('items.show', $item) }}" class="text-sm text-gray-500 hover:underline">{{ __('Cancel') }}</a>
