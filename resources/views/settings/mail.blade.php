<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Mail') }}</h2>
    </x-slot>
    ...
    <div class="max-w-lg mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <form method="POST" action="{{ route('settings.mail.update') }}" class="bg-white rounded-lg shadow p-6 space-y-4">
            @csrf
            ...
            <div>
                ...
                <select id="mail_encryption" name="mail_encryption" class="mt-1 block w-full rounded-md border-gray-300">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">{{ __('Leave empty to keep the current password.') }}</p>
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-3">
            <h3 class="text-sm font-medium text-gray-700">{{ __('Send a test email') }}</h3>
            <p class="text-sm text-gray-500">{{ __('Sends using whatever is currently filled in above, without saving it first.') }}</p>
            <form method="POST" action="{{ route('settings.mail.test') }}" class="flex flex-wrap gap-2">
                ...
                <input type="email" name="test_email" required placeholder="{{ __('Send to this address') }}" value="{{ old('test_email', auth()->user()->email) }}" class="flex-1 min-w-[200px] rounded-md border-gray-300 text-sm">
                <x-secondary-button type="submit">{{ __('Send test email') }}</x-secondary-button>
            </form>
            <p class="text-xs text-gray-400">{{ __('Uses the currently saved password unless you also typed a new one above.') }}</p>
        </div>
    </div>
</x-app-layout>
