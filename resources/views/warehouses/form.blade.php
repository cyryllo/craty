<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ $warehouse->exists ? 'Edytuj magazyn' : 'Nowy magazyn' }}</h2>
    </x-slot>
    ...
    <div class="max-w-lg mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <form method="POST" action="{{ $warehouse->exists ? route('warehouses.update', $warehouse) : route('warehouses.store') }}" class="bg-white rounded-lg shadow p-6 space-y-4">
            ...
            <div>
                <x-input-label for="code" value="Kod (używany w kodzie lokalizacji, np. M1)" />
            ...
            <div>
                <x-input-label for="address" value="Adres / opis" />
                <x-text-input id="address" name="address" class="mt-1 block w-full" value="{{ old('address', $warehouse->address) }}" />
            </div>

            <div class="flex items-center justify-end gap-3 pt-2 border-t border-gray-100">
                <a href="{{ route('warehouses.index') }}" class="text-sm text-gray-500 hover:underline">Anuluj</a>
